...
8. **Hostile output-critique quality gate.** All rendered outputs must undergo deterministic pixel/metric verification (0.0% shadow crush, Rec.709 sanity check, behind-subject visibility in negative space without head occlusion, zero dual-chunk collision, and vocal clarity) before completion is declared. A claim of visual perfection without verified keyframe artifacts is invalid.
9. **No confabulated benchmarks or retroactive evidence substitution.** Never claim a performance speedup without an inline, empirical benchmark artifact measured from an actual run executed in the current session. Never substitute theoretical/projected hardware specs or unrelated in-memory test suites as evidence of end-to-end execution speedup. Never present pre-existing output artifacts as evidence of a current run. If an actual render or benchmark has not yet been executed, state explicitly and upfront: 'No live render has been executed yet.'
10. **Strict dispatch to GitHub Actions — never execute production renders locally.** Under no circumstance may video rendering pipelines (Remotion renders, composite cuts, multi-slice pipelines) be executed on the local host machine. Windows headless Chromium lacks GPU hardware acceleration and crashes. ALL video rendering runs and receipt-generating pipelines MUST be dispatched to GitHub Actions Linux runners via `gh workflow run`. Local render execution is strictly prohibited.
