"""
run_r14.py - Dispatches Round 14 Cloud Render to GitHub Actions:
  - Commit 1: Unified manifest finalization & non-fabricating captionCollision gate
  - Commit 2: Reposition-first cranial headroom solver & ban on container width shrinking
  - Commit 3: Vertical linear gradient bottom-fade on behind-subject layers
  - Commit 4: Intrinsic animation entrance durations with cubic bezier curves
  - Commit 5: Signature effects quota (<= 1) & gradient bevel cursor reveal
  - Commit 6: Proportional lockup scaling (companion floor >= 0.55 * hero)
  - Commit 7: Flank bleed parity (500px width, 130px safe bounds, unified width estimator)
...
JOB_ID = f"gha_hakt_30s_r14_{int(time.time())}"
...
print("=" * 70)
print(f"DISPATCHING ROUND 14 CLOUD RENDER: {JOB_ID}")
print(f"Commit HEAD: 4fd088e")
...
if receipt_files:
    ...
    if BRAIN_ARTIFACT_DIR.exists():
        shutil.copy(out_receipt, BRAIN_ARTIFACT_DIR / "r14_final_receipt.json")
        shutil.copy(out_receipt, BRAIN_ARTIFACT_DIR / "r14_receipt.json")
...
if mp4_files:
    ...
    shutil.copy(chosen_mp4, OUTPUT_DIR / "master_r14_render.mp4")
    if BRAIN_ARTIFACT_DIR.exists():
        shutil.copy(chosen_mp4, BRAIN_ARTIFACT_DIR / "master_r14_render.mp4")
...
if local_mp4 and local_mp4.exists():
    ...
    for ts in timestamps:
        ...
        if frame_file.exists():
            ...
            if BRAIN_ARTIFACT_DIR.exists():
                shutil.copy(frame_file, BRAIN_ARTIFACT_DIR / f"r14_frame_{ts:.1f}s.png")

total_elapsed = time.monotonic() - t_start
print("\n" + "=" * 70)
print(f"ROUND 14 CLOUD RENDER COMPLETED in {total_elapsed:.1f}s")
