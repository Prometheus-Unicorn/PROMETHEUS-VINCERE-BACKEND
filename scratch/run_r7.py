"""
run_r7.py - Dispatches Round 7 cloud render to GitHub Actions, monitors execution,
downloads the receipt and master MP4, and extracts key verification frames.
"""
import base64
import json
import os
...
JOB_ID = f"gha_hakt_30s_r7_{int(time.time())}"
...
payload = {
    ...
    "metadata": {
        "pipeline": "minirun",
        "jobName": "pbd_30s_hakt_gha_r7",
        "lookId": "teal_and_orange_blockbuster",
    },
    "design": {
        "aspectRatio": "9:16",
...
print("=" * 70)
print(f"DISPATCHING ROUND 7 CLOUD RENDER: {JOB_ID}")
print(f"Commit: 0b94ef41cd384acfceecd69405120692c7e1a184")
print(f"Look: teal_and_orange_blockbuster | Motif: electric_cyan")
print("=" * 70)
...
# Record latest run id before triggering
    ...
    if status_res.returncode == 0:
        ...
        if status == "completed":
            ...
            if conclusion != "success":
                print(f"[poll] ERROR: workflow failed (conclusion={conclusion})", flush=True)
                sys.exit(1)
            break
...
if local_mp4 and local_mp4.exists():
    ...
    timestamps = [1.8, 3.8, 4.6, 8.6, 15.0, 22.5, 28.0]
...
print("\n" + "=" * 70)
print(f"ROUND 7 CLOUD RENDER COMPLETED in {total_elapsed:.1f}s")
